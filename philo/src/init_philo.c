...
/*   Updated: 2025/02/22 09:27:30 by stetrel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_philosophers(t_simulation *simulation)
{
	...
	while (i < simulation->data.nb_philo)
	{
		...
		simulation->philos[i].left_fork = &simulation->fork[(i + swap)
			% simulation->data.nb_philo];
		simulation->philos[i].right_fork = &simulation->fork[(i + 1 - swap)
			% simulation->data.nb_philo];
...
void	sim_init(t_simulation *simulation)
{
	...
	pthread_create(&mon, NULL, monitor, simulation);
